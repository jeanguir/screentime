...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['screenshot'], $_POST['user_id'])) {
    ...
    date_default_timezone_set('Asia/Novosibirsk'); // Устанавливаем часовой пояс +7
    $time = date('H:i:s');
    ...
    if ($check !== false) {
        ...
        if ($imageFileType === 'png') {
            if (move_uploaded_file($_FILES['screenshot']['tmp_name'], $targetFile)) {
                ...
                $stmt->bind_param("isss", $user_id, $targetFile, $create_day, $time); // Привязываем user_id, targetFile и date_time
                $stmt->execute();
